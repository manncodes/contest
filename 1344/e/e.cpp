...
const int MX=200200;
int T,n,m,i,x,y,z,pos,cnt,le,ri,h,fi[MX],lst[MX],s[MX],t[MX],ch[MX],cht[MX],a[18][MX],npw[MX],L[MX],R[MX],A[MX];
ll j;
...
int main() {
  ...
  dfs(1,0);
  //x=0; for (i=1; i<=T; i++) printf("%d %d = %d\n",x,i,a[x][i]);
  ...
  for (i=1; i<=m; i++) {
    ...
    for (j=1; j!=s[i]; j=y) {
      ...
      if (pos<fi[y] || pos>lst[y]) {
        ...
        //printf("change %d from %d to %d\n",j,y,g[j][ri]);
        seg[cnt++]={z?t[z]+d[j]+1:1,t[i]+d[j]};
        //printf(" SEG [%d %d]\n",int(seg[cnt-1].first),int(seg[cnt-1].second));
        ch[j]=ri;
        y=g[j][ri];
      }
    }
