...
int main() {
  ...
  cin.tie(0);
  const int BITS = 61;
  vector<vector<set<long long>>> all(2, vector<set<long long>>(BITS));
  vector<vector<set<long long>>> any(2, vector<set<long long>>(BITS));
  for (int rot = 0; rot < 2; rot++) {
    auto Add = [&](long long from, long long to) {
      int bits = (from == to ? 0 : 64 - __builtin_clzll(from ^ to));
      assert(to - from + 1 == (1LL << bits));
      all[rot][bits].insert(from);
    };
    int n;
    cin >> n;
    for (int i = 0; i < n; i++) {
  ...
  for (int k = BITS - 1; k >= 0; k--) {
    ...
    for (long long z : v) {
      segs.emplace_back(z, z + (1LL << k) - 1);
    }
  }
  Mint ans = 0;
  auto Add = [&](long long from, long long to) {
    ans += Mint(from + to) * Mint(to - from + 1) / 2; 
  };
