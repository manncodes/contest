...
int main() {
	...
	while (T--) {
		...
		for (int i = 0; i < N; i++) vals[i+1] = vals[i] + A[i];
		...
		binary_indexed_tree<int> bit(M, -2*N-2);
		std::vector<int> dp_val(M, -N-1);

		int64_t cur_pref = 0;
		for (int i = 0; i <= N; i++) {
			if (i) cur_pref += A[i-1];
			int v = int(std::lower_bound(vals.begin(), vals.end(), cur_pref) - vals.begin());
			assert(vals[v] == cur_pref);
			int cur = i == 0 ? 0 : dp[i-1] - 1;
			cur = std::max(cur, dp_val[v]);
			for (auto a : bit.prefix(v)) {
				cur = std::max(cur, a + i);
			}
			dp_val[v] = std::max(dp_val[v], cur);
			for (auto& a : bit.suffix(v)) {
				a = std::max(a, cur - i);
			}
			dp[i] = cur;
		}
