...
int main() {
	...
	std::vector<num> a(N+1);
	for (int sz = (N+1)/2; sz+1 <= N; sz++) {
		a[sz] = fact[N-sz-1];
	}
	a = ecnerwala::fft::fft_multiply<num>(a, ifact);

	std::vector<num> ans(N);
	num sub_prob = 0;
	for (int i = N-1; i > 0; i--) {
		num prob = a[N-i] * fact[N-i-1];
		ans[i] = prob * num(i) - sub_prob;
		sub_prob += prob;
	}
	ans[0] = fact[N-1] - std::accumulate(ans.begin()+1, ans.end(), num(0));
	for (int i = 0; i < N; i++) {
		cout << ans[i] << " \n"[i+1==N];
	}
